...
$proxy = 'false';
    ...
    if($mode === 'StudentCouncilandLocalCouncil') {
        ...
        if ($currDT > $electionEnd) {
            ...
            $stmtVoter = $pdo->prepare('SELECT * FROM tb_voter WHERE campus= :campus AND stud_id= :username');
            $stmtVoter->execute(['campus' => $campus, 'username' => $username]);
